...
- 不要自行清空 `data/tmp/group-tasks/`，runner 會處理；你只負責寫入自己負責方向的 task 檔。
- 本次可能只被要求處理單一 direction，若 prompt 結尾有「本次執行範圍限制」，就只切該方向、只驗該方向 100 檔。
...
細分類規則（必讀）：
- 分類前先讀 `data/taxonomy.json`，category 名稱優先使用其中已有的 canonical 名稱。只有確認沒有對應主題時，才自行創建新名稱；新名稱仍需符合下方細分規則。
