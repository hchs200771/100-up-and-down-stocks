...
### Step 1.68 — 融資餘額／維持率 + 外資選擇權（可與其他 script 平行）

```
npx tsx scripts/fetch-margin-options.ts
```

輸出 `data/margin-options-latest.json`（當日快照）與 `data/margin-history.json`（逐日融資序列，圖表疊圖用）。跟盤後主流程沒有相依，與 Step 1.5／1.6／1.65／1.66 同批平行丟。

**融資維持率是自己算的，不是官方數字。** 交易所只公布融資餘額張數與金額，沒有整體維持率。這裡用標準定義算：

```
維持率 = Σ(個股融資餘額張數 × 當日收盤價 × 1000) ÷ 融資餘額金額
```

券商公布值算法相同但成分略有出入，會差零點幾個百分點，**不要拿去逐位比對**。看的是趨勢與 166%（追繳）／130%（斷頭）兩條線。儀表板只在低於 166% 才變色。

**只算上市。** 上櫃只公布融資餘額張數、不公布金額，分母湊不出來；硬用上市的金額配上櫃的張數會算出假數字。上櫃張數另外列出當參考。

**外資選擇權用「未平倉」不是「當日交易」**（當日交易含大量價差單與隔日沖，方向性意義弱），而且**四個象限一定要並列**：
...
賣方是收權利金、賭「不會漲過去／不會跌破」，所以 Put 賣方偏多。**只看「外資買了多少 Call」會把避險部位讀成看多**，這是最常見的誤讀，不要把版面簡化成只講其中一格。

兩個端點的坑：

- **`MI_MARGN` 不會自動退到上一個交易日**（不像 MI_INDEX），假日或收盤前查會回空。script 會自己往回找最多 7 天。
- **TAIFEX `callsAndPutsDateDown` 只吃單日查詢**：起訖日不同會回一個帶 alert 的 HTML 頁而不是 CSV（實測 08/21~08/28、08/27~08/28 都失敗，08/28~08/28 才成功）。所以只能一天一天問，湊到兩個交易日算日增減。CSV 是 **Big5**，要用 `TextDecoder("big5")`，直接 `res.text()` 會整份亂碼。

要回補歷史融資序列：`npx tsx scripts/fetch-margin-options.ts --backfill 75`（一天兩個請求，75 個日曆日約兩分鐘）。
...
`renderRetailTrend` 是**雙軌渲染**，兩條路都要維護：

- 伺服器端先畫一張靜態 SVG（散戶長條 + 加權指數線 + 融資線都畫上去），**信件端看到的就是它**，沒有 JS 也完整。
- 網頁端 JS 接手後打開「顯示哪些資料」的 checkbox 與 hover 的十字線＋數值框，勾選改變時整張重畫（Y 軸跟著只剩下的序列重新縮放）。

規則：

- **checkbox 與 hover 提示預設 `display:none`，由 JS 打開。** 信件沒有 JS，一排點不動的核取方塊比沒有更糟。
- **同時勾兩條以上折線時不畫右軸刻度**，只畫線。三條序列（加權指數、融資餘額、維持率）量級完全不同，硬擠會疊在一起看不懂；要讀數值就 hover。
...
2a. **數字只能寫查到的，不能推估。** 尤其月營收：台灣上市櫃公司的月營收在**次月 10 日前**才公布，所以今天是 <tradingDate>，**本月的營收數字並不存在**，不准寫（連「預計」「估計」也不行）。最近一筆可用的是上個月的營收，且要 10 號之後才有。寫之前先自己算一次：這個月份的營收公布了嗎？
2b. 寫完檢查數字有沒有自相矛盾（例：說「創新高」但列出的數字比前一期小）。
...
`assemble-analysis.ts` 會呼叫 `scripts/check-story-facts.ts`，把**還沒公布的月營收**整句刪掉並印出警告。看到警告不用手動處理，但要知道發生了什麼。

**為什麼需要它**：台灣月營收在次月 10 日前才公布，所以 8/28 的報告裡不可能有「8 月營收」。實際發生過（2026-08-28）：

> 「京元電作為測試代工龍頭，**8月營收衝破31億元，年增31.9%**，連七月39.91億元年增36.75%的佳績後再創新高。」

七月那半句是真的，八月整句是編的，而且自相矛盾——31 億比 39.91 億少，不可能是「再創新高」。同一天還有金居的「8月預計月增7%至10.8億元」。

**為什麼不能只靠 prompt**：寫故事的是 haiku，prompt 已經寫了「不要硬湊」，它還是會在查不到時填一個看起來合理的數字。**這種錯誤讀起來完全正常**（有公司、有數字、有年增率），人工每天逐段抓不現實。prompt 規則（Step 4 的 2a/2b）是第一道防線，這支是第二道。

**處理方式是整句刪除，不是改寫**——改寫等於再編一次。少一句話的故事遠好過一個假數字。代價是同一句裡的正確資訊會一起消失（金居那句的 7 月營收是真的），這是刻意接受的取捨。

規則有兩條，都定義在 `check-story-facts.ts`：直接寫「N月營收」；以及句中提到營收、且未公布月份後 15 字內出現金額或增減幅（抓「8月預計月增7%至10.8億元」這種沒把「營收」接在月份後面的寫法）。第二條刻意收窄條件，避免誤殺「8月28日拉漲停」這類單純的日期敘述。
